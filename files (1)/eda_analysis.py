...
def detect_outliers(df: pd.DataFrame, columns: list, z_thresh: float = 2.5) -> pd.DataFrame:
    """
    Formal outlier detection using two standard, independent methods:
      - Z-score: |z| > z_thresh (default 2.5)
      - IQR (Tukey): outside [Q1 - 1.5*IQR, Q3 + 1.5*IQR]
    A row is reported per (Trial, column) flagged by either method, with
    both methods' verdicts shown, so a genuine outlier (flagged by both)
    can be distinguished from a borderline call (flagged by only one).
    """
    rows = []
    for col in columns:
        s = df[col]
        mean, std = s.mean(), s.std()
        z = (s - mean) / std if std > 0 else s * 0
        ...
        for idx in df.index[flagged]:
            rows.append({
                "Trial": df.loc[idx, "Trial"] if "Trial" in df.columns else idx,
                "column": col,
                "value": s.loc[idx],
                "z_score": round(z.loc[idx], 2),
                "flagged_by_zscore": bool(z_flag.loc[idx]),
                "flagged_by_iqr": bool(iqr_flag.loc[idx]),
                "flagged_by_both": bool(z_flag.loc[idx] and iqr_flag.loc[idx]),
            })
    return pd.DataFrame(rows).sort_values(
        ["flagged_by_both", "z_score"], ascending=[False, False], key=lambda s: s if s.dtype != bool else s.astype(int)
    ) if rows else pd.DataFrame(columns=["Trial", "column", "value", "z_score", "flagged_by_zscore", "flagged_by_iqr", "flagged_by_both"])
...
def run_eda(df_valid: pd.DataFrame, feature_cols: list, target_cols: list, out_prefix: str = "aas_eda"):
    ...
    outliers = detect_outliers(df_valid, all_cols)
    outliers.to_csv(f"{out_prefix}_outliers.csv", index=False)
    print(f"  [EDA] saved outlier flags -> {out_prefix}_outliers.csv")
    if not outliers.empty:
        both = outliers[outliers["flagged_by_both"]]
        if not both.empty:
            print("  [EDA] Outliers flagged by BOTH z-score and IQR methods (highest confidence):")
            print("  " + both.to_string(index=False).replace("\n", "\n  "))
        else:
            print("  [EDA] No outliers flagged by both methods simultaneously.")
    else:
        print("  [EDA] No outliers detected.")

    return {"stats": stats_df, "correlation": corr, "strong_correlations": strong_corr, "outliers": outliers}
