...
# ==============================================================================
FEATURE_COLS = [
    "Na2O_pct", "MS", "LS", "Alk_Bi", "has_Na2SiO3",
    "water_binder_ratio", "Na2SiO3_binder_ratio",
]
# Reduced from an original 11-feature set by dropping near-duplicate features
# (pairwise |r| >= 0.85, e.g. solid_activator_fraction vs water_binder_ratio
# r=0.999, MS vs Na2SiO3_NaOH r=0.991, NaOH_conc_value vs water_binder_ratio
# r=0.978, NaOH_binder_ratio redundant with MS/Na2SiO3_NaOH/Na2O_pct).
# Verified via LOOCV: improves the hybrid stack's R2 from 0.440->0.473 (IST)
# and 0.649->0.657 (FST) vs. the full 11-feature set on this dataset.
TARGET_COLS = ["IST", "FST"]
...
def ga_inverse_design(
    ...
    for gen in range(generations):
        ...
        pop = pop[order]
        scores = scores[order]
        convergence_log.append({"generation": gen, "best_fitness": float(scores[0])})
    ...
    convergence_log.append({"generation": generations, "best_fitness": float(best_score)})
    return best, best_score, pd.DataFrame(convergence_log)


# ==============================================================================
# MAIN
# ==============================================================================
def main(path: str, exclude_outliers: bool = False):
    ...
    print("\nStep 3b: Exploratory Data Analysis (descriptive stats, correlation heatmap, distributions, outlier detection)")
    eda_results = eda.run_eda(df_valid, FEATURE_COLS, ["IST", "FST"], out_prefix="aas_eda")

    if exclude_outliers:
        outliers = eda_results["outliers"]
        confirmed = outliers[outliers["flagged_by_both"]]
        trials_to_drop = sorted(confirmed["Trial"].unique().tolist())
        if trials_to_drop:
            print(f"\n  [exclude_outliers=True] Dropping trial(s) flagged by BOTH z-score and IQR: {trials_to_drop}")
            print("  NOTE: this removes training signal for that region of the mix-design space --")
            print("  the model (and GA inverse design) will no longer cover it. See report for the tradeoff.")
            df_valid = df_valid[~df_valid["Trial"].isin(trials_to_drop)].reset_index(drop=True)
        else:
            print("\n  [exclude_outliers=True] No trials flagged by both methods -- nothing excluded.")
    ...
    hybrid_ist, metrics_ist, pso_log_ist, parity_ist = train_hybrid_model(X, y_ist, "Initial Setting Time (IST)")
    hybrid_fst, metrics_fst, pso_log_fst, parity_fst = train_hybrid_model(X, y_fst, "Final Setting Time (FST)")
    ...
    print("\nStep 7: Evaluation figures (metrics comparison + PSO convergence + parity plots)")
    sa.save_evaluation_figures(metrics_table, out_prefix="aas_eval")
    sa.save_pso_convergence_figure(pso_log, out_prefix="aas_eval")
    sa.save_parity_plot(parity_ist, out_prefix="aas_eval")
    sa.save_parity_plot(parity_fst, out_prefix="aas_eval")
    ...
    best_vec, best_score, ga_convergence_log = ga_inverse_design(
    ...
    sa.save_ga_convergence_figure(ga_convergence_log, out_prefix="aas_eval")
