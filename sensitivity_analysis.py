...
def save_parity_plot(parity_data: dict, out_prefix: str = "aas_eval"):
    """
    Actual vs. predicted scatter plot (parity plot) for one target's LOOCV
    predictions from the hybrid stack, with the 1:1 reference line and R2
    annotated. A standard regression-evaluation figure.
    """
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    label = parity_data["target"]
    y_true = parity_data["y_true"]
    y_pred = parity_data["y_pred"]
    r2 = r2_score(y_true, y_pred)

    fig, ax = plt.subplots(figsize=(5.5, 5.5))
    ax.scatter(y_true, y_pred, color="#4C72B0", edgecolor="white", s=70, zorder=3)
    lims = [min(y_true.min(), y_pred.min()) - 5, max(y_true.max(), y_pred.max()) + 5]
    ax.plot(lims, lims, "--", color="gray", label="1:1 line", zorder=1)
    ax.set_xlim(lims)
    ax.set_ylim(lims)
    ax.set_xlabel(f"Actual {label} (min)")
    ax.set_ylabel(f"Predicted {label} (min, LOOCV)")
    ax.set_title(f"Parity plot -- Hybrid Stack ({label})\nR2 = {r2:.3f}")
    ax.legend()
    plt.tight_layout()
    safe_label = label.split(" ")[0].replace("(", "").replace(")", "")
    out_path = f"{out_prefix}_parity_{safe_label}.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
    print(f"  [Eval] saved parity plot -> {out_path}")
    return out_path


def save_ga_convergence_figure(ga_log, out_prefix: str = "aas_eval"):
    """Best-fitness-so-far vs generation for the GA inverse-design search."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.plot(ga_log["generation"], ga_log["best_fitness"], marker="o", color="#8172B2")
    ax.set_xlabel("GA generation")
    ax.set_ylabel("Best fitness (penalty, lower = better)")
    ax.set_title("GA convergence -- inverse mix-design search")
    plt.tight_layout()
    out_path = f"{out_prefix}_ga_convergence.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
    print(f"  [Eval] saved GA convergence figure -> {out_path}")
    return out_path
